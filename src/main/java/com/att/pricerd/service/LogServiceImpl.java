		    	...
		    	if(histActInstList != null && histActInstList.size() > 0 )
		    	{
		    		for(HistoricActivityInstance  currHistoricActivityInstance : histActInstList)
		    		{
		    					...
		    					&& currHistoricActivityInstance.getEndTime() != null)
		    			{
		    				...
		    				/*PerformanceTracking.addInvokeServiceTrack((PerformanceTrackingBean) context.getHttpServletRequest().getAttribute("PERFORMANCE_TRACKER_BEAN"),
		    						currHistoricActivityInstance.getActivityName(), Long.valueOf(startDate.getTime()), Long.valueOf(endDate.getTime()), actState,
		    						500, 1000) ;*/
		    		...
		    		for(HistoricActivityInstanceEventEntity actiEvent : historyEventList)
 	    		    {
						...
						/* PerformanceTracking.addInvokeServiceTrack((PerformanceTrackingBean)context.getHttpServletRequest().getAttribute("PERFORMANCE_TRACKER_BEAN"),
								 actName, Long.valueOf(startTime), Long.valueOf(endTime),activityState,
				 	   					500, 1000) ;*/
						 message = "Log Entry Created";
 	    		    }
		    	}
	    			...
	    			for(String currHistEvent : histEventArray)
	    			{
	    				if(currHistEvent != null && currHistEvent.contains(":"))
	    				{
	    					...
	    					if(custHistEventArray != null && custHistEventArray.length > 3)
	    					{
	    						...
	    						/*PerformanceTracking.addInvokeServiceTrack((PerformanceTrackingBean) context.getHttpServletRequest().getAttribute("PERFORMANCE_TRACKER_BEAN"),
	    								actName, Long.valueOf(startTime), Long.valueOf(endTime), actState,
	    	    						500, 1000) ;*/
	    					}
	    				}
	    			}
